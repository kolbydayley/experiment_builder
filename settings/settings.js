...
class SettingsManager {
    ...
    if (Object.keys(this.customTemplates).length === 0) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <p style="font-size: 12px; margin-top: 4px;">Click "Add New Template" to create your first template</p>
        </div>
      `;
      return;
    }

    container.innerHTML = Object.entries(this.customTemplates).map(([id, template]) => `
  ...
  duplicateTemplate(templateId) {
    ...
    while (this.customTemplates[finalId]) {
