...
class ContextBuilder {
   ...
   * @param {Object} options - { includeScreenshot, maxProximityElements, maxStructureElements }
   */
  buildContext(selectedElement = null, options = {}) {
    ...
    const context = {
      mode: selectedElement ? 'element-focused' : 'full-page',
      primary: [],
      proximity: [],
      structure: [],
    ...
    if (selectedElement) {
      ...
      context.structure = this.capturePageStructure(opts.maxStructureElements);
      context.metadata.focusPath = this.getElementPath(selectedElement);
    } else {
      // FULL-PAGE MODE
      context.primary = this.captureTopInteractiveElements(15); // Top 15 interactive elements
   ...
   */
  captureTopInteractiveElements(maxCount) {
